...
int segmentTreeQuery(int *tree, int left, int right, int segmentLeft, int segmentRight, int v) {
    if ((left == segmentLeft) && (right == segmentRight))
        ...
        if (right <= mid)
            return segmentTreeQuery(tree, left, right, segmentLeft, mid, 2 * v + 1);
        else {
            if (left > mid)
                return segmentTreeQuery(tree, left, right, mid + 1, segmentRight, 2 * v + 2);
            else
                return max(segmentTreeQuery(tree, left, mid, segmentLeft, mid, 2 * v + 1),
                           segmentTreeQuery(tree, mid + 1, right, mid + 1, segmentRight, 2 * v + 2));
        }
    }
}
